...
# ===== Overall Report (Insights & Forecast) — ROW-LEVEL version =====
import pandas as _or_pd

def _or__coerce_date(s):
    return _or_pd.to_datetime(s, errors="coerce", dayfirst=True, utc=False)

def _or__first_nonnull_col(df, candidates):
    # Exact, case-insensitive, normalized, partial
    for c in candidates:
        if c in df.columns: return c
    _lower = {col.lower(): col for col in df.columns}
    for c in candidates:
        if c.lower() in _lower: return _lower[c.lower()]
    import re as _re
    norm = lambda x: _re.sub(r"[^a-z0-9]", "", str(x).lower())
    _norm_map = {norm(col): col for col in df.columns}
    for c in candidates:
        key = norm(c)
        if key in _norm_map: return _norm_map[key]
    for c in candidates:
        key = norm(c)
...
def _or__resolve_referral_mask(df, deal_source_col):
    if (deal_source_col is None) or (deal_source_col not in df.columns):
        return _or_pd.Series(False, index=df.index)
    return df[deal_source_col].astype(str).str.lower().str.contains("referral", na=False)

def _or__compute_overall_report_rows(df, start, end, mode="MTD", track_value="Both", cols=None):
    cols = cols or {}
    ...
    # Series
    deals_created = _or__coerce_date(df[create_date]) if create_date else _or_pd.Series([], dtype="datetime64[ns]")
    enroll       = _or__coerce_date(df[enroll_date]) if enroll_date else _or_pd.Series([], dtype="datetime64[ns]")
    first_s      = _or__coerce_date(df[first_cal_sched]) if first_cal_sched else _or_pd.Series([], dtype="datetime64[ns]")
    resch_s      = _or__coerce_date(df[cal_resched]) if cal_resched else _or_pd.Series([], dtype="datetime64[ns]")
    cal_done_s   = _or__coerce_date(df[cal_done]) if cal_done else _or_pd.Series([], dtype="datetime64[ns]")
    cal_book_s   = _or__coerce_date(df[cal_booking]) if cal_booking else _or_pd.Series([], dtype="datetime64[ns]")
    cl_s         = _or__coerce_date(df[closed_lost_trigger]) if closed_lost_trigger else _or_pd.Series([], dtype="datetime64[ns]")

    # KPIs (row-level counts with Mode awareness)
    # Base masks
    deals_created_mask = deals_created.notna() & _or__in_window(deals_created, start, end)
    enroll_mask        = enroll.notna()        & _or__in_window(enroll,        start, end)

    # Enrollment Deals: always by Create Date in window
    enrollment_deals = int(deals_created_mask.sum())

    # Enrollments: MTD requires Payment in window AND Create in window; Cohort only Payment in window
    if str(mode).strip().lower() == "mtd":
        enrollments = int((enroll_mask & deals_created_mask).sum())
    else:
        enrollments = int(enroll_mask.sum())

    # Trials: row counts where (First Cal Scheduled OR Calibration Rescheduled) is in window
    trials_mask = (
        (first_s.notna() & _or__in_window(first_s, start, end)) |
        (resch_s.notna() & _or__in_window(resch_s, start, end))
    )
    trials = int(trials_mask.sum())

    # Calibration Done
    cal_done_mask = cal_done_s.notna() & _or__in_window(cal_done_s, start, end)
    calibration_done = int(cal_done_mask.sum())

    # Referral Generated (by Create Date)
    ref_mask = _or__resolve_referral_mask(df, deal_source)
    referral_generated = int((ref_mask & deals_created_mask).sum())

    # Sales-Generated Referral (by Create Date)
    if ris and (ris in df.columns):
        ris_mask = df[ris].astype(str).str.strip().str.lower() == "sales generated"
    else:
        ris_mask = _or_pd.Series(False, index=df.index)
    sales_generated_referral = int((ris_mask & deals_created_mask).sum())

    # Rates
    # Referral Enrollment (Rate):
    #   Denominator: referral & Create in window (same for both modes)
    #   Numerator (MTD): referral & Payment in window & Create in window
    #   Numerator (Cohort): referral & Payment in window (ignore Create)
    ref_den = int((ref_mask & deals_created_mask).sum())
    if str(mode).strip().lower() == "mtd":
        ref_num = int((ref_mask & enroll_mask & deals_created_mask).sum())
    else:
        ref_num = int((ref_mask & enroll_mask).sum())
    referral_enroll_rate = _or__pretty_pct(ref_num, ref_den)

    # Sales-Generated Referral Enrollment (Rate)
    ris_den = int((ris_mask & deals_created_mask).sum())
    if str(mode).strip().lower() == "mtd":
        ris_num = int((ris_mask & enroll_mask & deals_created_mask).sum())
    else:
        ris_num = int((ris_mask & enroll_mask).sum())
    sales_generated_enroll_rate = _or__pretty_pct(ris_num, ris_den)

    # Triggers
    cal_booking_mask = cal_book_s.notna() & _or__in_window(cal_book_s, start, end)
    cal_booking_count = int(cal_booking_mask.sum())

    cl_mask = cl_s.notna() & _or__in_window(cl_s, start, end)
    closed_lost_trigger_count = int(cl_mask.sum())
...
def render__INSIGHTS_OVERALL_REPORT__hook(st, df, start_ts, end_ts, mode_choice, track_choice):
    try:
        # Use the row-level compute function
        metrics = _or__compute_overall_report_rows(df, start_ts, end_ts, mode=mode_choice, track_value=track_choice)
        labels = [
        ...
        return {}
# ===== End Overall Report ROW-LEVEL version =====


# === Overall Report pill renderer (Insights & Forecast) ===
try:
    _m = st.session_state.get('nav_master', '')
    _v = st.session_state.get('nav_sub', '')
    if _m == "Insights & Forecast" and _v == "Overall Report":
        st.subheader("Insights & Forecast — Overall Report")
        from datetime import date, timedelta
        import pandas as pd
        _mode  = st.radio("Mode", ["MTD","Cohort"], index=0, horizontal=True, key="or_mode_kpi")
        _scope = st.radio("Date scope", ["Today","Yesterday","This Month","Last Month","Custom"], index=2, horizontal=True, key="or_scope_kpi")
        today = date.today()
        if _scope == "Today":
            _start, _end = pd.Timestamp(today), pd.Timestamp(today)
        elif _scope == "Yesterday":
            yd = today - timedelta(days=1); _start, _end = pd.Timestamp(yd), pd.Timestamp(yd)
        elif _scope == "This Month":
            _start, _end = pd.Timestamp(today.replace(day=1)), pd.Timestamp(today)
        elif _scope == "Last Month":
            first_this = today.replace(day=1); last_month_end = first_this - timedelta(days=1)
            _start, _end = pd.Timestamp(last_month_end.replace(day=1)), pd.Timestamp(last_month_end)
        else:
            c1, c2 = st.columns(2)
            with c1: _start = pd.to_datetime(st.date_input("Start", value=today.replace(day=1), key="or_start_date"), dayfirst=True)
            with c2: _end   = pd.to_datetime(st.date_input("End", value=today, key="or_end_date"), dayfirst=True)

        _track_val = "Both"
        for _cand in ["Track","Pipeline","Program","Program Category","Category"]:
            if _cand in df.columns:
                opts = ["Both"] + sorted(df[_cand].dropna().astype(str).str.strip().unique().tolist())
                _track_val = st.selectbox("Track", options=opts, index=0, key="or_track_kpi")
                break

        render__INSIGHTS_OVERALL_REPORT__hook(st, df, _start, _end, _mode, _track_val)
except Exception:
    pass
