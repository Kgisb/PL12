...
# --- Global control: exclude invalid deals ---
import streamlit as st  # ensure st is in scope here
st.sidebar.checkbox("Exclude '1.2 Invalid deal' rows globally", value=True, key="exclude_invalid_deals_global")
...
def load_csv(path: str) -> pd.DataFrame:
    ...
    # Apply global invalid-deal filter if enabled
    try:
        if st.session_state.get("exclude_invalid_deals_global", True):
            if "Deal Stage" in df.columns:
                _stage = df["Deal Stage"].astype(str).str.strip().str.lower()
                df = df[_stage != "1.2 invalid deal"]
    except Exception:
        pass
    # Normalize columns
...
try:
    ...
    if _m == "Insights & Forecast" and _v == "Overall Report":
        ...
        df = _apply_global_invalid_filter(df)
