...
def _render_marketing_talk_time(df):
    ...
    dff = dff[~bad_rows].copy()
        ...
        with c1:
            start_date = _tt_st.date_input("Start date", key="tt_start")
        with c2:
            end_date = _tt_st.date_input("End date", key="tt_end")
        with c3:
            pass
        ...
        with c4:
            pick_agents = _tt_st.multiselect("Agent(s)", options=agents, default=agents, key="tt_agents")
        with c5:
            pick_countries = _tt_st.multiselect("Country", options=countries, default=countries, key="tt_countries")
        with c6:
            pick_status = _tt_st.multiselect("Call Status", options=statuses, default=statuses, key="tt_status")
        with c7:
            pick_type = _tt_st.multiselect("Call Type", options=ctypes, default=ctypes, key="tt_type")

        # Apply filters
        dfv = dff.copy()
        if "Caller" in dfv.columns and pick_agents:
            dfv = dfv[dfv["Caller"].isin(pick_agents)]
        if "Country Name" in dfv.columns and pick_countries:
            dfv = dfv[dfv["Country Name"].isin(pick_countries)]
        if "Call Status" in dfv.columns and pick_status:
            dfv = dfv[dfv["Call Status"].isin(pick_status)]
        if "Call Type" in dfv.columns and pick_type:
            dfv = dfv[dfv["Call Type"].isin(pick_type)]
        if "_tt_dt" in dfv.columns:
            dfv = dfv[(dfv["_tt_dt"].dt.date >= start_date) & (dfv["_tt_dt"].dt.date <= end_date)]
