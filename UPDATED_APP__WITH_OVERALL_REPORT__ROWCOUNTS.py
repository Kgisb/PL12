...
def _or__compute_overall_report_rows(df, start, end, mode="MTD", track_value="Both", cols=None):
    ...
    # KPIs (row-level counts)
    enrollment_deals  = int((_or__in_window(deals_created, start, end) & deals_created.notna()).sum())
    enrollments       = int((_or__in_window(enroll,        start, end) & enroll.notna()).sum())
    trials            = int(( (first_s.notna() & _or__in_window(first_s, start, end)) | (resch_s.notna() & _or__in_window(resch_s, start, end)) ).sum())
    calibration_done  = int((_or__in_window(cal_done_s, start, end) & cal_done_s.notna()).sum())

    ref_mask = _or__resolve_referral_mask(df, deal_source)
    referral_generated = int((ref_mask & deals_created.notna() & _or__in_window(deals_created, start, end)).sum())
    ...
    sales_generated_referral = int((ris_mask & deals_created.notna() & _or__in_window(deals_created, start, end)).sum())

    ref_enroll = int((ref_mask & enroll.notna() & _or__in_window(enroll, start, end)).sum())
    ref_create = int((ref_mask & deals_created.notna() & _or__in_window(deals_created, start, end)).sum())
    referral_enroll_rate = _or__pretty_pct(ref_enroll, ref_create)

    ris_enroll = int((ris_mask & enroll.notna() & _or__in_window(enroll, start, end)).sum())
    ris_create = sales_generated_referral
    sales_generated_enroll_rate = _or__pretty_pct(ris_enroll, ris_create)

    cal_booking_count = int((_or__in_window(cal_book_s, start, end) & cal_book_s.notna()).sum())
    closed_lost_trigger_count = int((_or__in_window(cl_s, start, end) & cl_s.notna()).sum())
