...
# ===== Overall Report (Insights & Forecast) — Injected CLEAN =====
...
def _or__unique_deals_by_date(df, deal_id_col, date_col, start, end):
    ...
    m = _or__in_window(s, start, end)
    return set(df.loc[m & s.notna(), deal_id_col].astype(str))
...
def _or__compute_overall_report(df, start, end, mode="MTD", track_value="Both", cols=None):
    ...
    # Trials: First Calibration Scheduled OR Calibration Rescheduled (unique deals; same-day dedup implicitly handled)
    ...
    # KPIs
    deals_created_series = _or__coerce_date(df[create_date]) if create_date else _or_pd.Series([], dtype='datetime64[ns]')
    enrollment_deals = int((_or__in_window(deals_created_series, start, end) & deals_created_series.notna()).sum())

    enroll_ids          = _or__unique_deals_by_date(df, deal_id, enroll_date, start, end)
    enrollments         = len(enroll_ids)

    trials_first_ids    = _or__unique_deals_by_date(df, deal_id, first_cal_sched, start, end)
    trials_resched_ids  = _or__unique_deals_by_date(df, deal_id, cal_resched,    start, end)
    trials_ids          = trials_first_ids.union(trials_resched_ids)
    ...
    if ris and ris in df.columns:
        ris_df = df[df[ris].astype(str).str.strip().str.lower() == "sales generated"].copy()
    else:
        ris_df = df.iloc[0:0].copy()
    sales_gen_deals_ids = _or__unique_deals_by_date(ris_df, deal_id, create_date, start, end)
    sales_generated_referral = len(sales_gen_deals_ids)
    ...
    sales_generated_enroll_rate = _or__pretty_pct(len(ris_enroll_ids), len(sales_gen_deals_ids))
...
def render__INSIGHTS_OVERALL_REPORT__hook(st, df, start_ts, end_ts, mode_choice, track_choice):
    try:
        ...
        return {}
# ===== End Overall Report Injected CLEAN =====
