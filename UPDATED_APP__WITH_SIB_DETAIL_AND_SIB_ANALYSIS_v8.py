...
if view == "Funnel":
    def _funnel_tab():
        ...
        # --- Funnel pills (inside function for visibility) ---
        _fn_pill2 = st.radio("Funnel pills", ["None","Sib Detail","Sibling Analysis"], index=0, horizontal=True, key="fn_local_pill_inside")
        if _fn_pill2 == "Sib Detail":
            try:
                _render_funnel_sib_detail(df)
            except Exception as e:
                st.warning(f"Sib Detail failed: {e}")
        elif _fn_pill2 == "Sibling Analysis":
            try:
                _render_funnel_sibling_analysis(df)
            except Exception as e:
                st.warning(f"Sibling Analysis failed: {e}")
...
def _render_funnel_sib_detail(df_f: pd.DataFrame):
    ...
    if not create_col or not stage_col or not sib_col:
        st.info("Required columns not found (need ‘Create Date’, ‘Deal Stage’, and ‘Sibling Deal’).", icon="ℹ️"); return
    _df = df_f.copy(); _df["_CREATE_DT"] = pd.to_datetime(_df[create_col], errors="coerce", dayfirst=True, infer_datetime_format=True)
    ...
    if scope == "Today": start,end = today.normalize(), today.normalize()
    elif scope == "Yesterday": end = (today - pd.Timedelta(days=1)).normalize(); start=end
    elif scope == "This Month": start, end = today.replace(day=1).normalize(), today.normalize()
    elif scope == "Last Month":
        first_this = today.replace(day=1).normalize(); end = first_this - pd.Timedelta(days=1); start = end.replace(day=1).normalize()
    else:
        c = st.date_input("Custom range (Create Date)", (today.replace(day=1).date(), today.date()))
        if isinstance(c, tuple) and len(c)==2: start, end = pd.Timestamp(c[0]), pd.Timestamp(c[1])
        else: start, end = today.replace(day=1).normalize(), today.normalize()
    win = _df[_df["_CREATE_DT"].between(start, end, inclusive="both")].copy()
    if win.empty: st.warning("No rows in the selected window.", icon="⚠️"); return
    pivot = win.groupby([stage_col, sib_col]).size().rename("Count").reset_index()
    ...
    tbl = pivot.pivot_table(index=stage_col, columns=sib_col, values="Count", aggfunc="sum", fill_value=0).reindex(index=stage_order, columns=sib_order)
    tbl["Total"] = tbl.sum(axis=1); total_row = tbl.sum(axis=0).to_frame().T; total_row.index=["Total"]; table_out = pd.concat([tbl, total_row])
    st.dataframe((tbl.div(tbl["Total"].replace(0, np.nan), axis=0)*100).round(1).fillna(0).assign(Total=100.0) if show_pct else table_out, use_container_width=True)
    chart_df = pivot.copy()
    ...
    st.altair_chart(alt.Chart(chart_df).mark_bar().encode(
        ...
        tooltip=[stage_col, sib_col, "Count"]).properties(height=max(240, 28*len(stage_order)), width="container"), use_container_width=True)
...
# ==================================================
# Funnel & Movement — Sibling Analysis
def _render_funnel_sibling_analysis(df_f):
    import streamlit as st, pandas as pd, numpy as np, altair as alt
    from datetime import date
    st.markdown("### Sibling Analysis")
    def find_col(_df, candidates):
        for c in candidates:
            if c in _df.columns: return c
        cl = {c.lower(): c for c in _df.columns}
    ...
    stage_col  = find_col(df_f, ["Deal Stage","Stage"])
    sib_col    = find_col(df_f, ["Sibling Deal","Sibling deal","Sibling"])
    ac_col     = find_col(df_f, ["Student/Academic Counsellor","Academic Counsellor","Counsellor","Advisor","Student/Advisor"])
    if not create_col or not stage_col or not sib_col:
        st.info("Required columns not found (need ‘Create Date’, ‘Deal Stage’, and ‘Sibling Deal’).", icon="ℹ️"); return
    _df = df_f.copy(); _df["_CREATE_DT"] = pd.to_datetime(_df[create_col], errors="coerce", dayfirst=True, infer_datetime_format=True)
    _df = _df[_df[stage_col].astype(str).str.strip().str.lower() != "1.2 invalid deal(s)"]
    scope = st.selectbox("Date scope (Create Date)", ["Today","Yesterday","This Month","Last Month","Custom"], index=2, key="sibA_scope")
    today = pd.Timestamp(date.today())
    if scope == "Today": start,end = today.normalize(), today.normalize()
    elif scope == "Yesterday": end = (today - pd.Timedelta(days=1)).normalize(); start=end
    elif scope == "This Month": start, end = today.replace(day=1).normalize(), today.normalize()
        ...
        c = st.date_input("Custom range (Create Date)", (today.replace(day=1).date(), today.date()), key="sibA_custom")
        if isinstance(c, tuple) and len(c)==2: start, end = pd.Timestamp(c[0]), pd.Timestamp(c[1])
        else: start, end = today.replace(day=1).normalize(), today.normalize()
    win = _df[_df["_CREATE_DT"].between(start, end, inclusive="both")].copy()
    if win.empty: st.warning("No rows in the selected window.", icon="⚠️"); return
    part = st.radio("View", ["Part A — Deal Stage × Sibling Deal","Part B — Counsellor views"], index=0, horizontal=True, key="sibA_part")
    if part.startswith("Part A"):
        grp = win.groupby([stage_col, sib_col]).size().rename("Count").reset_index()
        if grp.empty: st.info("No Sibling Deal distribution for this window.", icon="ℹ️"); return
        stage_order = grp.groupby(stage_col)["Count"].sum().sort_values(ascending=False).index.tolist()
        sib_order   = grp.groupby(sib_col)["Count"].sum().sort_values(ascending=False).index.tolist()
        with st.expander("Standing — top Sibling Deal categories (overall)", expanded=True):
            topN = st.slider("Show top N categories", 3, max(3, len(sib_order)), min(8, len(sib_order)), key="sibA_topN")
            standing = grp.groupby(sib_col)["Count"].sum().sort_values(ascending=False).rename("Count").reset_index()
            standing["Share %"] = (standing["Count"]/standing["Count"].sum()*100).round(1); st.dataframe(standing.head(topN), use_container_width=True)
        tbl = grp.pivot_table(index=stage_col, columns=sib_col, values="Count", aggfunc="sum", fill_value=0).reindex(index=stage_order, columns=sib_order)
        tbl["Total"] = tbl.sum(axis=1); total_row = tbl.sum(axis=0).to_frame().T; total_row.index=["Total"]; table_out = pd.concat([tbl, total_row])
        show_pct = st.checkbox("Show % within Deal Stage", value=False, key="sibA_rowpct")
        if show_pct:
            pct = (tbl.div(tbl["Total"].replace(0, np.nan), axis=0)*100).round(1).fillna(0); pct["Total"] = 100.0
        ...
        st.altair_chart(
            alt.Chart(grp).mark_bar().encode(
                ...
                x=alt.X("Count:Q", title="Count"),
                color=alt.Color(f"{sib_col}:N", legend=alt.Legend(title="Sibling Deal")),
                tooltip=[stage_col, sib_col, "Count"]).properties(height=max(240, 28*len(stage_order)), width="container"),
            use_container_width=True)
        st.download_button("Download table (CSV) — Part A", data=table_out.reset_index().to_csv(index=False).encode("utf-8"),
                           file_name="sibling_analysis_partA.csv", mime="text/csv")
    else:
        ac_col = ac_col  # already resolved
        if not ac_col: st.info("No Academic Counsellor column found.", icon="ℹ️"); return
        viewB = st.radio("Counsellor view", ["Counsellor × Sibling Deal","Counsellor × Deal Stage"], index=0, horizontal=True, key="sibB_view")
        if viewB == "Counsellor × Sibling Deal":
            grpB = win.groupby([ac_col, sib_col]).size().rename("Count").reset_index()
            topK = st.slider("Top counsellors by total volume", 5, 50, 15, key="sibB_topK")
            top_cs = grpB.groupby(ac_col)["Count"].sum().sort_values(ascending=False).head(topK).index.tolist()
            grpB = grpB[grpB[ac_col].isin(top_cs)]
            ac_order  = grpB.groupby(ac_col)["Count"].sum().sort_values(ascending=False).index.tolist()
            sib_order = grpB.groupby(sib_col)["Count"].sum().sort_values(ascending=False).index.tolist()
            tblB = grpB.pivot_table(index=ac_col, columns=sib_col, values="Count", aggfunc="sum", fill_value=0).reindex(index=ac_order, columns=sib_order)
            tblB["Total"] = tblB.sum(axis=1); st.dataframe(tblB, use_container_width=True)
            st.altair_chart(alt.Chart(grpB).mark_bar().encode(
                y=alt.Y(f"{ac_col}:N", sort=ac_order, title="Academic Counsellor"),
                x=alt.X("Count:Q", title="Count"),
                color=alt.Color(f"{sib_col}:N", legend=alt.Legend(title="Sibling Deal")),
                tooltip=[ac_col, sib_col, "Count"]).properties(height=max(260, 22*len(ac_order)), width="container"),
                use_container_width=True)
            with st.expander("Drill into a counsellor"):
                pick = st.selectbox("Choose counsellor", ac_order, index=0, key="sibB_pick")
                sub = win[win[ac_col] == pick]
                if not sub.empty:
                    d = sub.groupby([stage_col, sib_col]).size().rename("Count").reset_index()
                    if not d.empty: st.dataframe(d.pivot_table(index=stage_col, columns=sib_col, values="Count", aggfunc="sum", fill_value=0), use_container_width=True)
            st.download_button("Download table (CSV) — Part B (Counsellor × Sibling Deal)", data=tblB.reset_index().to_csv(index=False).encode("utf-8"),
                               file_name="sibling_analysis_partB_cns_x_sib.csv", mime="text/csv")
        else:
            grpB2 = win.groupby([ac_col, stage_col]).size().rename("Count").reset_index()
            topK2 = st.slider("Top counsellors by total volume", 5, 50, 15, key="sibB2_topK")
            top_cs2 = grpB2.groupby(ac_col)["Count"].sum().sort_values(ascending=False).head(topK2).index.tolist()
            grpB2 = grpB2[grpB2[ac_col].isin(top_cs2)]
            ac_order2  = grpB2.groupby(ac_col)["Count"].sum().sort_values(ascending=False).index.tolist()
            stage_order2 = grpB2.groupby(stage_col)["Count"].sum().sort_values(ascending=False).index.tolist()
            tblB2 = grpB2.pivot_table(index=ac_col, columns=stage_col, values="Count", aggfunc="sum", fill_value=0).reindex(index=ac_order2, columns=stage_order2)
            tblB2["Total"] = tblB2.sum(axis=1); st.dataframe(tblB2, use_container_width=True)
            st.altair_chart(alt.Chart(grpB2).mark_bar().encode(
                y=alt.Y(f"{ac_col}:N", sort=ac_order2, title="Academic Counsellor"),
                x=alt.X("Count:Q", title="Count"),
                color=alt.Color(f"{stage_col}:N", legend=alt.Legend(title="Deal Stage")),
                tooltip=[ac_col, stage_col, "Count"]).properties(height=max(260, 22*len(ac_order2)), width="container"),
                use_container_width=True)
            st.download_button("Download table (CSV) — Part B (Counsellor × Deal Stage)", data=tblB2.reset_index().to_csv(index=False).encode("utf-8"),
                               file_name="sibling_analysis_partB_cns_x_stage.csv", mime="text/csv")
