...
def _or__compute_overall_report(df, start, end, mode="MTD", track_value="Both", cols=None):
    ...
    first_cal_sched = cols.get("first_cal_sched") or _or__first_nonnull_col(df, ["First Calibration Scheduled Date"])
    cal_resched = cols.get("cal_resched") or _or__first_nonnull_col(df, ["Calibration Rescheduled Date"])
    cal_done = cols.get("cal_done") or _or__first_nonnull_col(df, ["Calibration Done Date","Trial Done Date","Calibration Completed Date"])
    cal_booking = cols.get("cal_booking") or _or__first_nonnull_col(df, ["Calibration Booking Date"])
    closed_lost_trigger = cols.get("closed_lost_trigger") or _or__first_nonnull_col(df, ["Closed Lost Trigger Date"])
