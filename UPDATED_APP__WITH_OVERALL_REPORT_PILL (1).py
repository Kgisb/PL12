...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Insights & Forecast": ["Predictibility","Business Projection","Buying Propensity","80-20","Trend & Analysis","Heatmap","Bubble Explorer","Master Graph", "Overall Report"],
...
# ===== Overall Report (Insights & Forecast) — Injected =====
import pandas as pd
def _or__coerce_date(s):
    return pd.to_datetime(s, errors="coerce", dayfirst=True, utc=False)

def _or__first_nonnull_col(df, candidates):
    # Exact first
    for c in candidates:
        if c in df.columns:
            return c
    ...
    for c in candidates:
        ...
        if lc in lower_map:
            return lower_map[lc]
    # Normalized alnum comparison (remove non-alphanumerics)
    import re as _re
    ...
    # Partial contains on normalized tokens
    for c in candidates:
        cn = _norm(c)
...
def _or__in_window(series, start, end):
    return series.between(start, end, inclusive="both")

def _or__unique_deals_by_date(df, deal_id_col, date_col, start, end):
    if (deal_id_col is None) or (date_col is None) or (deal_id_col not in df.columns) or (date_col not in df.columns):
        return set()
    s = _or__coerce_date(df[date_col])
    mask = _or__in_window(s, start, end)
    return set(df.loc[mask & s.notna(), deal_id_col].astype(str))

def _or__pretty_pct(num, den):
    if den == 0: return "—"
    return f"{(num/den)*100:,.1f}% ({num} / {den})"

def _or__resolve_referral_mask(df, deal_source_col):
    if (deal_source_col is None) or (deal_source_col not in df.columns):
        import pandas as pd
        return pd.Series(False, index=df.index)
    src = df[deal_source_col].astype(str).str.lower()
    return src.str.contains("referral", na=False)

def _or__compute_overall_report(df, start, end, mode="MTD", track_value="Both", cols=None):
    cols = cols or {}
    deal_id = cols.get("deal_id") or _or__first_nonnull_col(df, ["Deal ID","HubSpot Deal ID","Deal Name","DealId","DealName"])
    deal_stage = cols.get("deal_stage") or _or__first_nonnull_col(df, ["Deal Stage","DealStage"])
    track_col = cols.get("track") or _or__first_nonnull_col(df, ["Track","Pipeline","Program","Program Category","Category"])
    create_date = cols.get("create_date") or _or__first_nonnull_col(df, ["Create Date","Deal Create Date","Created Date"])
    enroll_date = cols.get("enroll_date") or _or__first_nonnull_col(df, ["Payment Received Date","Enrollment Date","Payment Date"])
    first_cal_sched = cols.get("first_cal_sched") or _or__first_nonnull_col(df, [
    "First Calibration Scheduled Date","First Calibration Schedule Date","First Calibration Date","Calibration First Scheduled Date"
])
cal_resched = cols.get("cal_resched") or _or__first_nonnull_col(df, [
    "Calibration Rescheduled Date","Calibration Reschedule Date","Rescheduled Date","Calibration Re-scheduled Date"
])
cal_done = cols.get("cal_done") or _or__first_nonnull_col(df, [
    "Calibration Done Date","Trial Done Date","Calibration Completed Date"
])
 or _or__first_nonnull_col(df, ["Calibration Done Date","Trial Done Date","Calibration Completed Date"])
    cal_booking = cols.get("cal_booking") or _or__first_nonnull_col(df, [
    "Calibration Booking Date","Calibration Booked Date","Calibration Booking","Booked Calibration Date"
])
closed_lost_trigger = cols.get("closed_lost_trigger") or _or__first_nonnull_col(df, [
    "Closed Lost Trigger Date","Closed-Lost Trigger Date","Closed Lost - Trigger Date","Closed Lost Triggered Date","Closed Lost (Trigger) Date"
])
 or _or__first_nonnull_col(df, ["Closed Lost Trigger Date"])
    deal_source = cols.get("deal_source") or _or__first_nonnull_col(df, ["JetLearn Deal Source","Deal Source","Source","Original Source"])
    ris = cols.get("ris") or _or__first_nonnull_col(df, ["Referral Intent Source","RIS","Referral Source Detail"])

    # drop invalid
    if deal_stage and (deal_stage in df.columns):
        df = df[df[deal_stage].astype(str).str.strip().str.lower() != "1.2 invalid deal"]

    if track_col and (track_col in df.columns) and track_value and (track_value.lower() != "both"):
        df = df[df[track_col].astype(str).str.strip().str.lower() == track_value.strip().lower()]

    deals_created_ids = _or__unique_deals_by_date(df, deal_id, create_date, start, end)
    enrollment_deals = len(deals_created_ids)

    enroll_ids = _or__unique_deals_by_date(df, deal_id, enroll_date, start, end)
    enrollments = len(enroll_ids)

    trials_first = _or__unique_deals_by_date(df, deal_id, first_cal_sched, start, end)
    trials_resch = _or__unique_deals_by_date(df, deal_id, cal_resched, start, end)
    trials_ids = trials_first.union(trials_resch)
    trials = len(trials_ids)

    cal_done_ids = _or__unique_deals_by_date(df, deal_id, cal_done, start, end)
    calibration_done = len(cal_done_ids)

    ref_mask = _or__resolve_referral_mask(df, deal_source)
    ref_df = df[ref_mask].copy()
    referral_deals_ids = _or__unique_deals_by_date(ref_df, deal_id, create_date, start, end)
    referral_generated = len(referral_deals_ids)

    if ris and (ris in df.columns):
        ris_mask = df[ris].astype(str).str.strip().str.lower() == "sales generated"
        ris_df = df[ris_mask].copy()
    else:
        ris_df = df.iloc[0:0].copy()

    sales_generated_deals_ids = _or__unique_deals_by_date(ris_df, deal_id, create_date, start, end)
    sales_generated_referral = len(sales_generated_deals_ids)

    ref_enroll_ids = _or__unique_deals_by_date(ref_df, deal_id, enroll_date, start, end)
    referral_enroll_rate = _or__pretty_pct(len(ref_enroll_ids), len(referral_deals_ids))

    ris_enroll_ids = _or__unique_deals_by_date(ris_df, deal_id, enroll_date, start, end)
    sales_generated_enroll_rate = _or__pretty_pct(len(ris_enroll_ids), len(sales_generated_deals_ids))

    cal_booking_ids = _or__unique_deals_by_date(df, deal_id, cal_booking, start, end)
    cal_booking_count = len(cal_booking_ids)

    cl_trigger_ids = _or__unique_deals_by_date(df, deal_id, closed_lost_trigger, start, end)
    closed_lost_trigger_count = len(cl_trigger_ids)

    return {
        "Enrollment Deals": enrollment_deals,
        "Enrollments": enrollments,
        "Trials": trials,
        "Calibration Done": calibration_done,
        "Referral Generated": referral_generated,
        "Sales-Generated Referral": sales_generated_referral,
        "Referral Enrollment (Rate)": referral_enroll_rate,
        "Sales-Generated Referral Enrollment (Rate)": sales_generated_enroll_rate,
        "[Trigger] Calibration Booking Date": cal_booking_count,
        "[Deal Stage] Closed Lost Trigger Date": closed_lost_trigger_count,
    }

def _or__render(st, df, start, end, mode="MTD", track_value="Both"):
    
metrics = _or__compute_overall_report(df, start, end, mode=mode, track_value=track_value, cols=None)
# Column presence notices for the two new KPIs
_missing_notes = []
if "[Trigger] Calibration Booking Date" in metrics and metrics["[Trigger] Calibration Booking Date"] == 0:
    # We can't directly know if the column was missing; re-resolve and report if absent
    _cbook = _or__first_nonnull_col(df, ["Calibration Booking Date","Calibration Booked Date","Calibration Booking","Booked Calibration Date"])
    if not _cbook:
        _missing_notes.append("Could not find a column for Calibration Booking Date (tried: Calibration Booking Date / Calibration Booked Date).")
_cl_col = _or__first_nonnull_col(df, ["Closed Lost Trigger Date","Closed-Lost Trigger Date","Closed Lost - Trigger Date","Closed Lost Triggered Date","Closed Lost (Trigger) Date"])
if "[Deal Stage] Closed Lost Trigger Date" in metrics and metrics["[Deal Stage] Closed Lost Trigger Date"] == 0 and not _cl_col:
    _missing_notes.append("Could not find a column for Closed Lost Trigger Date (tried common variants).")
if _missing_notes:
    import streamlit as st
    for _msg in _missing_notes:
        st.info(_msg)
labels =
 [
        "Enrollment Deals","Enrollments","Trials","Calibration Done",
        "Referral Generated","Sales-Generated Referral",
        "Referral Enrollment (Rate)","Sales-Generated Referral Enrollment (Rate)",
        "[Trigger] Calibration Booking Date","[Deal Stage] Closed Lost Trigger Date"
    ]
    cols = st.columns(5)
    for i, lab in enumerate(labels):
        with cols[i % 5]:
            st.metric(lab, metrics.get(lab, "—"))
    return metrics
# ===== End Overall Report Injected =====



# === Hook function to call Overall Report within your Insights & Forecast tab ===
def render__INSIGHTS_OVERALL_REPORT__hook(st, df, start_ts, end_ts, mode_choice, track_choice):
    try:
        return _or__render(st, df, start_ts, end_ts, mode=mode_choice, track_value=track_choice)
    except Exception as e:
        import traceback
        st.error(f"Overall Report failed: {e}")
        st.code(traceback.format_exc())
        return {}
# === End hook ===


# === Overall Report pill renderer (Insights & Forecast) ===
try:
    _this_master = st.session_state.get('nav_master', '')
    _this_sub = st.session_state.get('nav_sub', '')
    if _this_master == "Insights & Forecast" and _this_sub == "Overall Report":
        st.subheader("Insights & Forecast — Overall Report")
        from datetime import date, timedelta
        # Controls local to this pill
        _or_mode = st.radio("Mode", ["MTD","Cohort"], index=0, horizontal=True, key="or_mode_kpi")
        _or_scope = st.radio("Date scope", ["Today","Yesterday","This Month","Last Month","Custom"], index=2, horizontal=True, key="or_scope_kpi")
        today = date.today()
        if _or_scope == "Today":
            _start = pd.Timestamp(today)
            _end = pd.Timestamp(today)
        elif _or_scope == "Yesterday":
            yd = today - timedelta(days=1)
            _start = pd.Timestamp(yd); _end = pd.Timestamp(yd)
        elif _or_scope == "This Month":
            _start = pd.Timestamp(today.replace(day=1)); _end = pd.Timestamp(today)
        elif _or_scope == "Last Month":
            first_this = today.replace(day=1)
            last_month_end = first_this - timedelta(days=1)
            _start = pd.Timestamp(last_month_end.replace(day=1)); _end = pd.Timestamp(last_month_end)
        else:
            c1, c2 = st.columns(2)
            with c1:
                _start = pd.to_datetime(st.date_input("Start", value=today.replace(day=1), key="or_start_date"), dayfirst=True)
            with c2:
                _end = pd.to_datetime(st.date_input("End", value=today, key="or_end_date"), dayfirst=True)

        # Track dropdown (if present)
        _track_candidates = [c for c in ["Track","Pipeline","Program","Program Category","Category"] if c in df.columns]
        _track_val = "Both"
        if _track_candidates:
            uniq = df[_track_candidates[0]].dropna().astype(str).str.strip().unique().tolist()
            _opts = ["Both"] + sorted(list(uniq))
            _track_val = st.selectbox("Track", options=_opts, index=0, key="or_track_kpi")

        # Render KPIs
        try:
            render__INSIGHTS_OVERALL_REPORT__hook(st, df, _start, _end, _or_mode, _track_val)
        except Exception as _e:
            import traceback
            st.error(f"Overall Report failed: {_e}")
            st.code(traceback.format_exc())
except Exception:
    pass
# === End Overall Report pill renderer ===
